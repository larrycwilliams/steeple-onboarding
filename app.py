...
APP_VERSION = "3.57"   # shown in the header so you can tell a stale process at a glance
...
def preview_file(pid):
    ...
    # The sandbox goes on SVG only, which is the one thing here that can carry
    # script. It came off PDFs because the Launch Week Owners page is a form
    # you are meant to TYPE INTO, and a sandboxed response can leave a
    # browser's PDF viewer read-only -- disabling the only thing the document
    # exists for. It bought nothing anyway: every file this route serves was
    # generated by this app, and the one piece of outside content in them is
    # the partner's own logo, embedded as an image.
...
def discovery_add_call(sid):
    _discovery_or_404(sid)
    call, error = discovery.add_call(
        sid, request.form.get("who", ""), request.form.get("note", ""),
        discovery.combine_when(request.form.get("when_date", ""),
                               request.form.get("when_time", "")))
